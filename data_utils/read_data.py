...
if __name__=='__main__':
    path = 'D:/USTC/Research/Code_project/full-shape'
    ...
    for idx, (data, label,affordance,  _, _) in enumerate(val_loader):
        x = data  # [B, N, 3]
        y = label # [B, N, 18]  ps: 18 means the number of affordance classes
        print(affordance)
        pdb.set_trace()
        # affordance_list = [0]*18
        # for i in range(x.size(1)):
        #     for j in range(18):
        #         if(y[0][i][j]) != m:
        #             affordance_list[j] += 1
        # all_affordance.append(affordance_list)

        # if(idx == 50):
        #     print(all_affordance)
        #     break
